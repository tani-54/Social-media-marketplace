...
const ChatBox = () => {
    ...
    const { listing, isOpen, chatId } = useSelector(
        (state) => state.chat
    )
    ...
    // Load chat when listing changes
    useEffect(() => {
        if (listing) {
            fetchChat()
        }
   ...
   const handleSendMessage = async (e) => {
    ...
    if (!newMessage.trim() || isSending || !chat) {
        return
    }
    ...
    // Don't show ChatBox when chat is closed
